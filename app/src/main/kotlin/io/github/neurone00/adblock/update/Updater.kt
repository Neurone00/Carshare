package io.github.neurone00.adblock.update
 ...
 * record (i.e. from the second self-update on).
 */
object Updater {
    private const val TAG = "Updater"
    const val MANIFEST_URL = "https://github.com/Neurone00/Carshare/releases/download/adblock-latest/update.json"
    ...
    fun currentVersionCode(context: Context): Long {
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) pi.longVersionCode else @Suppress("DEPRECATION") pi.versionCode.toLong()
    }
    ...
    private fun download(url: String, dest: File, expectedSize: Long, progress: (Int) -> Unit) {
        ...
        try {
            ...
            if (!tmp.renameTo(dest)) { dest.delete(); if (!tmp.renameTo(dest)) throw IOException("could not save update") }
        } finally {
            conn.disconnect()
        }
    }
